...
function handlePost():never{
 ...
 if($action==='patient'){
  ...
  $a=[val('fname',80,true),val('lname',100,true),$mobile,$national?:null,$dob,$sex,$email,val('address',2000),val('emergency_name',160),val('emergency_phone',20),val('notes',5000),isset($_POST['allow_portal'])?'YES':'NO'];
  if($id){if(!q('SELECT pid FROM patients WHERE pid=?',[$id])->fetchColumn())throw new DomainException('پرونده پیدا نشد.');q('UPDATE patients SET fname=?,lname=?,phone_cell=?,national_id=?,DOB=?,sex=?,email=?,address=?,emergency_name=?,emergency_phone=?,notes=?,allow_patient_portal=? WHERE pid=?',[...$a,$id]);}
  else {q('INSERT INTO patients(fname,lname,phone_cell,national_id,DOB,sex,email,address,emergency_name,emergency_phone,notes,allow_patient_portal) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)',$a);$id=(int)$db->lastInsertId();}
